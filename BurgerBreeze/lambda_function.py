...
def validate_order(slots):
    ...
    if not slots.get('BurgerFranchise'):
    ...
    franchise = slots['BurgerFranchise']['value']['originalValue'].lower()
    burger_type = slots['BurgerType']['value']['originalValue'].lower()

    if franchise == 'best burger':
        if burger_type not in best_burger_types:
    ...
    elif franchise == 'burger palace':
        if burger_type not in burger_palace_types:
    ...
    elif franchise == 'flaming burger':
        if burger_type not in flaming_burger_types:
...
def lambda_handler(event, context):
    ...
    # Extract intent and slots from event
    session_state = event.get('sessionState', {})
    intent = session_state.get('intent', {})
    slots = intent.get('slots', {})
    intent_name = intent.get('name', 'OrderBurger')
    ...
    # Determine the response based on the invocation source
    if event.get('invocationSource') == 'DialogCodeHook':
        ...
        response = {
            ...
            "messages": [
                {
                    "contentType": "PlainText",
                    "content": "Unexpected invocation source."
                }
            ]
        }
